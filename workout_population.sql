-- =============================================
-- MEGAMIND — Workout Population Script
-- Populates: ExerciseBodyRegion + DayExercise
-- Generated from: Tendon.txt (Q1) and Neural.txt (Q2)
-- Run after megamind_workout_schema_v2.sql
-- =============================================
...
-- Check ExerciseBodyRegion row count
SELECT COUNT(*) AS ExerciseBodyRegionRows FROM ExerciseBodyRegion;

-- Check DayExercise row count
SELECT COUNT(*) AS DayExerciseRows FROM DayExercise;

-- Preview DayExercise for Q1 Day 1
SELECT de.OrderLetter, e.ExerciseName, de.PrescribedSets,
       de.PrescribedReps, de.TimeRangeMin, de.TimeRangeMax,
       de.LoadKg, de.LoadType, de.RestSeconds, de.RestSecMax,
       de.GripSequence, de.IsClusterSet, de.Notes
FROM DayExercise de
JOIN Exercise e ON de.ExerciseID = e.ExerciseID
WHERE de.CycleDayID = 1
ORDER BY de.OrderLetter;

-- Preview ExerciseBodyRegion for Q1 Day 1 exercises
SELECT e.ExerciseName, br.RegionName, ebr.IsPrimary
FROM ExerciseBodyRegion ebr
JOIN Exercise e ON ebr.ExerciseID = e.ExerciseID
JOIN BodyRegion br ON ebr.RegionID = br.RegionID
WHERE ebr.ExerciseID IN (SELECT ExerciseID FROM DayExercise WHERE CycleDayID = 1)
ORDER BY e.ExerciseName, ebr.IsPrimary DESC;
